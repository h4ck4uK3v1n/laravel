...
<div class="row">
    ...
    <div class="card mb-4">
      <div class="card-header d-flex justify-content-between align-items-center">
        <h5 class="mb-0">Creando un nuevo tipo</h5> 
      </div>
      <div class="card-body">
        <form method="post" action="{{ route('pages-types-update') }}">
          ...
          <div class="mb-3">
            ...
            <input type="text" name="name" class="form-control" value="{{$type->name}}" id="basic-default-fullname"  />
          </div>
          <div class="mb-3">
            <label class="form-label" for="basic-default-company">Descripcion</label>
            <input type="text" name="description" class="form-control" value="{{$type->description}}"id="basic-default-email" />
          </div>
          <div class="mb-3">
            ...
            <select name="icon" class="selectpicker w-100 show-tick" id="selectpickerIcons" data-icon-base="bx" data-tick-icon="bx-check" data-style="btn-default">
              <option value="bx bx-link" @if($type->icon=='bx bx-link') selected @endif data-icon="bx bx-link">Tuberías y Conexiones</option>
              <option value="bx bx-slider-alt" @if($type->icon=='bx bx-slider-alt') selected @endif data-icon="bx bx-slider-alt">Válvulas y Controles</option>
              <option value="bx bx-drink" @if($type->icon=='bx bx-drink') selected @endif data-icon="bx bx-drink">Drenajes y Desagües</option>
              <option value="bx bx-wrench" @if($type->icon=='bx bx-wrench') selected @endif data-icon="bx bx-wrench">Herramientas de Plomería</option>
